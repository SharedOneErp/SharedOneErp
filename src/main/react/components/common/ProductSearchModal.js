...
function ProductSearchModal({ onClose, onProductSelect }) {
    ...
    // 🟡 searchQuery, searchCode, 카테고리 변경될 때마다 fetchProducts 호출
    useEffect(() => {
        fetchProducts();
    }, [searchCode, searchQuery, selectedCategory, currentPage]); // 카테고리 상태와 페이지 번호도 추가
    ...
    const fetchProducts = () => {
        axios.get('/api/products/productsFilter', {
            params: {
                ...
                productNm: searchQuery || null,                 // 상품명 필터
